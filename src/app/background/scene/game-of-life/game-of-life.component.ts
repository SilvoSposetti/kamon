...
export class GameOfLifeComponent implements OnInit, OnDestroy {
  ...
  private columns: number = 200;
  private rows: number;
  private cellSize: number;
  private grid: boolean[][] = [];
  private updateEachAmountOfFrame: number = 5;
  private frameCount: number = 0;
  private paddingValue: boolean = false;

  // each element of the grid contains 0 for black and 1 for white.
  ...
  private setup(): void {
    ...
    let value: boolean;
    let firstAndLastRow: boolean[] = [];
    for (let i = 0; i < this.columns + 2; i++) {
      firstAndLastRow.push(this.paddingValue);
    }
    this.grid.push(firstAndLastRow);
    for (let i = 0; i < this.rows; i++) {
      let row = [];
      row.push(this.paddingValue);
